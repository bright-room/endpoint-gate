...
2. **プロジェクト構成とガイドラインの読み込み** — coding.md, settings.gradle.kts, architecture.md
3. **ブランチの準備** — モードに応じて新規作成 or チェックアウト
4. **コードの実装** — 入力ソースに基づいて段階的に実装
5. **ビルド確認** — `spotlessApply` → モジュール部分ビルド → `check`
6. **コミット** — 変更内容を適切なメッセージでコミット
7. **Push / PR 作成** — モードに応じて Push のみ or PR 作成
